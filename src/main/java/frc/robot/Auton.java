...
public final class Auton extends SubsystemBase {
    ...
    private static final Map<String, Command> commands = Map.ofEntries(
        ...
        entry("1: Twice Mid and Back Right", new ParallelCommandGroup(
                zeroGyro(),                                                                                // brake to prevent drifting
                followPathCommand("GoToMidAndBack", true).andThen(() -> Drivetrain.getInstance().setControl(SwerveConstants.brake)),
            ...
            .andThen(followPathCommand("RestartMidRight", false),
                new InstantCommand(() -> launcher.setMode(Launcher.Mode.OFF), launcher),
                new ParallelCommandGroup(                                                              // brake to prevent drifting
                    followPathCommand("GoToMidAndBack", false).andThen(() -> Drivetrain.getInstance().setControl(SwerveConstants.brake)),
                    new WaitCommand(1).andThen(
                        new InstantCommand(() -> intake.setExtension(true), intake),
                        new InstantCommand(() -> intake.toggleSpin(), intake),
                        new WaitCommand(3.5),
        ...
        entry("2: Twice Mid and Back Left", new ParallelCommandGroup(
                zeroGyro(),                                                                                // brake to prevent drifting
                ...
                new InstantCommand(() -> intake.toggleSpin(), intake),
                new InstantCommand(() -> intake.setAgitation(false), intake))
            .andThen(followPathCommand("RestartMidLeft", false),
                new InstantCommand(() -> launcher.setMode(Launcher.Mode.OFF), launcher),
                new ParallelCommandGroup(                                                                // brake to prevent drifting
                    followPathCommand("GoToMidAndBack_Left", false).andThen(() -> Drivetrain.getInstance().setControl(SwerveConstants.brake)),
                    new WaitCommand(1).andThen(
                        new InstantCommand(() -> intake.setExtension(true), intake),
                        new InstantCommand(() -> intake.toggleSpin(), intake),
                        new WaitCommand(3.5),
    ...
    private static Command followPathCommand(String path, boolean firstPath) {
        return firstPath ? autoFactory.resetOdometry(path).andThen(autoFactory.trajectoryCmd(path))
